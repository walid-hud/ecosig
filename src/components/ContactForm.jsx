...
const   ContactForm = () => {
  ...
  const delayTest = ()=>{
    setBtnContent(()=><>sending<ClipLoader/></>)
    setTimeout(() => {
      setBtnContent(()=><>sent</>)
    }, 3000);
  }
  return (
    <div className="">
      ...
      <form className='flex flex-col mt-4 '>
        ...
        <input type="text" required name="full name" placeholder="Prénom et nom"  inputMode="text" id="name" className='outline-1 focus:outline-sky-600 rounded-md p-2' />


        <label htmlFor="email" className="text-sm mt-4 px-2 mb-[-2px] w-fit backdrop-opacity-0">
              Email<span className="text-red-600">*</span>
        </label>
        <input type='email' required name="Email" placeholder="Email" inputMode="text" id="email"
        className='outline-1 focus:outline-blue-700 rounded-md p-2  invalid:outline-red-500 ' />
       ...
       <label htmlFor="subject" className="text-sm mb-[-2px] mt-4 px-2 w-fit backdrop-opacity-0">sujet</label>
        ...
        <button type="submit" className='outline-1 outline-black bg-blue-500 active:scale-90 active:animate-bounce focus:outline-red-600 text-xl text-white flex items-center justify-center gap-4 py-2 px-4  rounded-md mb-10 md:mb-0 hover:bg-blue-700 transition duration-300  
        '
        onClick={()=>delayTest()}
